...
# ファイルをバックエンドに組み込み
try:
    yokohama_batting = pd.read_csv("data/yokohama_batting.csv").round(3)
    opponent_batting = pd.read_csv("data/opponent_batting.csv").round(3)
    yokohama_pitching = pd.read_csv("data/yokohama_pitching.csv").round(2)
    opponent_pitching = pd.read_csv("data/opponent_pitching.csv").round(2)
