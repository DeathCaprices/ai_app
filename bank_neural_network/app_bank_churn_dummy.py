...
def load_data():
    ...
    df = pd.DataFrame({
        ...
        "Balance": np.random.randint(0, 100000, size=n_samples),
        "EstimatedSalary": np.random.randint(20000, 150000, size=n_samples),
        "Geography": np.random.choice(["France", "Spain", "Germany"], size=n_samples),
        "Exited": np.random.randint(0, 2, size=n_samples)
    })
    return df
...
if model_type == "数値データモデル":
    ...
    model.fit(X_num_train, y_train, epochs=3, batch_size=8, verbose=0)
else:
    model = create_model_embedding()
    model.compile(optimizer="adam", loss="binary_crossentropy", metrics=["accuracy"])
    model.fit([X_num_train, X_cat_train], y_train, epochs=3, batch_size=8, verbose=0)
...
if input_mode == "サンプルから選ぶ":
    idx = st.number_input("サンプル行番号を選択 (0~19)", 0, 19, 0)
